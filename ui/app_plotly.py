...
def gen_waveform(interval):
    ...
    for i, k in enumerate(keys):
        ...
        if len(picks) > 0:
            picks["t"] = picks["timestamp"].apply(lambda x: (timestamp_seconds(x) - wave_time[k])/DELTAT + WAVE_WINDOW_LENGTH)
            ...
            trace = dict(
                ...
                hoverinfo="skip",
                mode="markers",
                marker={"color": "rgb(0,255,0)", 'line': {'color': 'rgb(0,255,0)', 'width': 2}, "size":20, "symbol":[42 for _ in range(len(x))]},
                name=None,
                showlegend=False
            )
            traces.append(trace)
            ...
            trace = dict(
                ...
                hoverinfo="skip",
                mode="markers",
                marker={"color": "rgb(255,0,0)", 'line': {'color': 'rgb(255,0,0)', 'width': 2}, "size":20, "symbol":[42 for _ in range(len(x))]},
                name=None,
                showlegend=False
            )
            traces.append(trace)
